...
[
	...
	["<t color='#FFE496'><img image='client\icons\r3f_lock.paa'/> Open Base Menu</t>", "addons\BoS\BoS_selectMenu.sqf", [cursorTarget], -97, false, false, "", "(alive cursorTarget) and (vehicle player == player) and (cursortarget iskindof 'Land_SatellitePhone_F') and (player distance cursortarget) < 5"],	
	["<t color='#FFE496'><img image='client\icons\take.paa'/> Hack Base</t>", "addons\BoS\BoS_hackBase.sqf", [cursorTarget], -97, false, false, "", "(alive cursorTarget) and ('MineDetector' in (items player)) and (cursorTarget getVariable ['ownerUID',''] != getPlayerUID player) and (vehicle player == player) and (cursortarget iskindof 'Land_SatellitePhone_F') and (player distance cursortarget) < 5"],	
	...
	["Chop Shop", "addons\scripts\chopShop.sqf", [], 51, false, false, "", "!(vehicle player == player) and (cursortarget iskindof 'Land_CarService_F') and (player distance cursortarget) < 15"], //Thanks Top
